...
uint16_t gfx.getLastJPEGwidth(), word;  // Returns the width of the Last JPEG decoded in pixels.
// Syntax	: gfx.getLastJPEGwidth();
// Usage	: gfx.getLastJPEGwidth();
// Notes 	: Returns the width of the Last JPEG decoded in pixels.
//

uint16_t gfx.getLastJPEGheight(), word;  // Returns the height of the Last JPEG decoded in pixels.
// Syntax	: gfx.getLastJPEGheight();
// Usage	: gfx.getLastJPEGheight();
// Notes 	: Returns the height of the Last JPEG decoded in pixels.
//

uint16_t gfx.getLastJPEGpaddingWidth(), word;  // Returns the last width of padding after the last decoded JPEG.
// Syntax	: gfx.getLastJPEGpaddingWidth();
// Usage	: gfx.getLastJPEGpaddingWidth();
// Notes 	: Returns the last width of padding after the last decoded JPEG.
//
...
void gfx.DrawFrameBufferAreaPPA(fbnum, x1, y1, x2, y2), void; // Uses the PPA accelerator to draw the rectangular area specified by the diagonal (x1, y1), (x2, y2) from the selected framebuffer fbnum
// Syntax	: gfx.DrawFrameBufferAreaPPA(fbnum, x1, y1, x2, y2);
// Usage	: gfx.DrawFrameBufferAreaPPA(3, 100, 100, 199, 199);
// Notes 	: Draw the rectangular area specified by the diagonal (x1, y1), (x2, y2) from the selected framebuffer fbnum to the current frame buffer
//
...
void gfx.DrawFrameBufferAreaXYPPA(fbnum, x1, y1, x2, y2, x, y), void; // Draw the rectangular area specified by the diagonal (x1, y1), (x2, y2) from the selected framebuffer fbnum at x, y position
// Syntax	: gfx.DrawFrameBufferAreaXYPPA(fbnum, x1, y1, x2, y2, x, y);
// Usage	: gfx.DrawFrameBufferAreaXYPPA(1, 200, 200, 399, 399, 100, 100);
// Notes	: Uses the PPA Accelerator to draw the rectangular area specified by the diagonal (x1, y1), (x2, y2) from the selected framebuffer fbnum at x, y position.
//

void gfx.DrawFrameBufferAreaScaleRotate(fb, fx, fy, fw, fh, deg, scW, scH, mirX, mirY, altX, altY, fb2), void; // Draw the rectangular area and apply scale, mirror and rotate actions
// Syntax	: gfx.DrawFrameBufferAreaScaleRotate(fb, fx, fy, fw, fh, deg, scW, scH, mirX, mirY, altX, altY, fb2);
// Usage	: gfx.DrawFrameBufferAreaScaleRotate(1, 200, 200, 100, 100, 90, 200, 200, true, false, 400, 100, -1);
// Notes	: if fb2 not assigned or set to -1 draw the output to current frame buffer.
//
...
//			: uSD is mounted at this stage and called separately. Optional parameters for startup behaviour
...
// Syntax	: gfx.getAngle();
// Usage	: gfx.getAngle();
// Notes	: returns the last calculated angle in Angular functions.
//
...
void gfx.CircleAA(x, y, r, t, colour), void; // Draw an anti aliaised circle with center at (x, y), with radius r and colour 'colour'.
// Syntax	: gfx.CircleAA(x, y, r, t, colour);
// Usage	: gfx.CircleAA(100, 120, 40, 5, LIME);
// Notes 	: Draw an anti aliaise circle at x, y, with radius r, thickness t and colour 'colour'.
//
...
void gfx.RectangleFilledPPA(x1, y1, x2, y2, colour), void; // Uses PPA accelerator to draws a filled rectangle having a diagonal with endpoints at (x1, y1) and (x2, y2) with colour 'colour'.
// Syntax	: gfx.RectangleFilledPPA(x1, y1, x2, y2, colour);
// Usage	: gfx.RectangleFilledPPA(10, 20, 110, 170, RED);
// Notes 	: Draws a filled rectangle having a diagonal with endpoints at (x1, y1) and (x2, y2) with colour 'colour'.
//

void gfx.Rectangle(x1, y1, x2, y2 [, w], colour), void; // Draws a rectangle having a diagonal with endpoints at (x1, y1) and (x2, y2) with colour 'colour'.
// Syntax	: gfx.Rectangle(x1, y1, x2, y2, colour);
// Syntax	: gfx.Rectangle(x1, y1, x2, y2, w, colour);
// Usage	: gfx.Rectangle(20, 50, 120, 160, MAGENTA);
// Usage	: gfx.Rectangle(20, 50, 120, 160, 6, MAGENTA);
// Notes 	: Draws a rectangle having a diagonal with endpoints at (x1, y1) and (x2, y2) with colour 'colour'.
//      	: Can have an optional line width for drawing thicker outline rectangles.
//
...
void gfx.RoundRectAA(x1, y1, x2, y2, r, thk, colour), void; // Draws a anti-aliaised rounded rectangle having a diagonal with endpoints at (x1, y1) and (x2, y2), corner radius 'r' and colour 'colour'.
// Syntax	: gfx.RoundRect(x1, y1, x2, y2, r, thk, colour);
// Usage	: gfx.RoundRect(50, 50, 200, 200, 25, 4, YELLOW);
// Notes 	: Draws an anti-aliaised rounded rectangle having a diagonal with endpoints at (x1, y1) and (x2, y2), corner radius 'r' line thickness thk and colour 'colour'.
//
...
void gfx.SetCanvasDimensions(int w, int h), void; // Sets the Canvas frame buffer size.
// Syntax	: gfx.SetCanvasDimensions(w, h);
// Usage	: gfx.SetCanvasDimensions(1280, 1280);
// Notes 	: Sets the canvas CANVAS_BUFFER_ARGB prior to using DrawToframebuffer command.
//          	: The Blend canvas can be larger than the displays size and moved over another frame
//          	: buffer.
//

void gfx.SetBlendCanvasDimensions(int w, int h), void; // Sets the Canvas frame buffer size.
// Syntax	: gfx.SetBlendCanvasDimensions(w, h);
// Usage	: gfx.SetBlendCanvasDimensions(1280, 1280);
// Notes 	: Sets the blend canvas CANVAS_BUFFER_ARGB prior to using DrawToframebuffer command.
//          	: The PrintImageFromFrameBuffercanvas can be larger than the displays size and moved over another frame
//          	: buffer.
//

void gfx.ShowCanvas(fb, fb2, [px, py, ]x, y, w, h, tx, ty), void; // Shows the Blended frame buffer to current framebuffer.
// Syntax	: gfx.ShowCanvas(fb, fb2, px, py, x, y, w, h, tx, ty);
// Usage	: gfx.ShowCanvas(1, CANVAS_BUFFER_ARGB, 0, 0, posX, posY, 800, 400, 0, 200);
// Notes 	: Shows the contents of blended frame buffers fb and fb2 using specified co-ordinates. 
//	 	: fb2 can be 16 bit or alpha 24 bit frame buffers.
// 	 	: px and py are the source frame buffer (bg) co-ordinates for start of frame buffer position and only used
// 		: when Blending
//

void gfx.UpdateBlendCanvas(), void; // Updates and shows changes made to the CANVAS_BUFFER_ARGB to the current frame buffer.
// Syntax	: gfx.UpdateBlendCanvas();
// Usage	: gfx.UpdateBlendCanvas();
// Notes 	: Updates only the CANVAS_BUFFER_ARGB and displays to current frame buffer. 
// Notes 	: gfx.ShowCanvas must be called before use.
//

void gfx.updateCanvas(), void; // Updates and shows changes made to the CANVAS_BUFFER to the current frame buffer.
// Syntax	: gfx.updateCanvas();
// Usage	: gfx.updateCanvas();
// Notes 	: Updates only the CANVAS_BUFFER and displays to current frame buffer. 
// Notes 	: gfx.ShowCanvas must be called before use.
//

void gfx.BlendAlphaBufferLevel(bool bl), void; // Sets the behaviour of alpha blending when writing CANVAS_BUFFER_ARGB
// Syntax	: gfx.BlendAlphaBufferLevel(bool bl);
// Usage	: gfx.BlendAlphaBufferLevel(ALPHA_BLEND);
// Usage	: gfx.BlendAlphaBufferLevel(ALPHA_OVERWRITE);
// Notes 	: Sets blending characteristics for the Blend Canvas when writing at a pixel level.
//

void gfx.PPAblendTransAlpha(btcol, btEN, balphVal,baEN), void; // Sets blending parmaeters for Blend Canvas.
// Syntax	: gfx.setJPEGinputBufferSize(btcol, btEN, balphVal,baEN);
// Usage	: gfx.setJPEGinputBufferSize(BLACK, true, 127 false);
// Notes 	: Sets blending characteristics for the Blend Canvas.
//

void gfx.setJPEGinputBufferSize(s), void; // Sets the input buffer size for JPEG decoding.
// Syntax	: gfx.setJPEGinputBufferSize(s);
// Usage	: gfx.setJPEGinputBufferSize(128 * 1024);
// Notes 	: Sets the JPEG decode input buffer to allocate space for larger JPEG images used on the CANVAS_BUFFER.
//          	: Must be used before gfx.begin().
//

void gfx.setJPEGoutputBufferDimension(w, h), void; // Sets the output buffer size for JPEG decoding.
// Syntax	: gfx.setJPEGinputBufferSize(w, h);
// Usage	: gfx.setJPEGinputBufferSize(1280, 1280);
// Notes 	: Sets the JPEG decode output buffer to required CANVAS_BUFFER size space for larger JPEG images used on the CANVAS_BUFFER.
//          	: Must be used before gfx.begin().
//

MJPEGvideoTimingReset()
void gfx.MJPEGvideoTimingReset(), void; // Resets all MJPEG video timing in between frames.
// Syntax	: gfx.MJPEGvideoTimingReset();
// Usage	: gfx.MJPEGvideoTimingReset();
// Notes 	: Resets all MJPEG video timing in between frames
//

void gfx.MJPEGkeepAspectRatio(kasp), void; // Enables Aspect ratio preservation to scaled playback window.
// Syntax	: gfx.MJPEGkeepAspectRatio(kasp);
// Usage	: gfx.MJPEGkeepAspectRatio(true);
// Notes 	: When set to true the PlayMJPEGnextFrame will perform rescaling to keep aspect ratio on first frame.
//          	: The area will be cleared to BLACK and video is positioned centrally.
//

void gfx.MJPEGTargetFPS(fps), void; // Sets the target frames per second for MJPEG videos.
// Syntax	: gfx.MJPEGTargetFPS(fps);
// Usage	: gfx.MJPEGTargetFPS(25);
// Notes 	: The PlayMJPEGnextFrame function uses this value to keep timing as close as possibe to target.
//          	: Can be used while video is in play. if more than 10 skipped frames is detected, target FPS will be automatically reduced.
//

int64_t gfx.PlayMJPEGnextFrame(), int64_t; // Displays next MJPEG video frame and returns frame number
// Syntax	: gfx.PlayMJPEGnextFrame();
// Usage	: gfx.PlayMJPEGnextFrame();
// Notes 	: Displays the next MJPEG video frame using parameters set when MJPEG was opened. 
//          	: The function will automatically adjust timing to try to keep the traget FPS.
//

void gfx.OpenMJPEGvideoFile(fname, x, y, TargetFPS, altw, alth, loop), void; // Opens an MJEPG video file stored on uSD with scaled output.
// Syntax	: gfx.OpenMJPEGvideoFile(fname, x, y, TargetFPS, altw, alth, loop);
// Usage	: gfx.OpenMJPEGvideoFile("MyVideo.mjpeg", 0, 0, 25, 640, 480, false);
// Notes 	: Opens an MJPEG video file and sets parameters for scaled playback.
//

void gfx.OpenMJPEGvideoArray(array, arraySize, x, y, TargetFPS, altw, alth, loop), void; // Opens an MJEPG video file stored in Flash or PSRAM with scaled output.
// Syntax	: gfx.OpenMJPEGvideoArray(array, arraySize, x, y, TargetFPS, altw, alth, loop);
// Usage	: gfx.OpenMJPEGvideoArray(MyVideoArray, MyVideoArray_size, 0, 0, 25, 640, 480, false);
// Notes 	: Opens an MJPEG video array from Flash or PSRAM and sets parameters for scaled playback.
//

void gfx.OpenMJPEGvideoStream(Sname, x, y, altw, alth), void; // Opens an MJEPG video stream URL.
// Syntax	: gfx.OpenMJPEGvideoStream(Sname, x, y, altw, alth);
// Usage	: gfx.OpenMJPEGvideoStream("http....port...", 0, 0, 640, 480);
// Notes 	: Opens an MJPEG video Stream using MJPEG video stream URL.
//


void gfx.CloseMJPEGvideo(), void; // Closes previously opened MJPEG video
// Syntax	: gfx.CloseMJPEGvideo();
// Usage	: gfx.CloseMJPEGvideo();
// Notes 	: Closes previously opened MJPEG video. 
//

bool gfx.OpenMJPEGvideoFile(filename, x, y, TargetFPS, loop), bool; // Opens MJPEG video file and sets playback parameters.
// Syntax	: gfx.Open4dGFX(filename, x, y, TargetFPS, loop);
// Usage	: gfx.Open4dGFX("VideoJPEG.mjpeg", 0, 0, 25, true);
// Notes 	: Opens MJPEG video file and sets position, target FPS, and whether auto restarts.
// Notes 	: Returns true if successfully opened.
//

uint32_t gfx.PlayMJPEGnextFrame(), uint32_t; // Plays the next frame of an opened MJPEG file.
// Syntax	: gfx.PlayMJPEGnextFrame();
// Usage	: gfx.PlayMJPEGnextFrame();
// Notes 	: Plays next MJPEG frame.
// Notes 	: Playback speed depends on size and MJPEG quality settings.
//
...
//      	: gfx.Open4dGFX("sample.gcj");
// Notes 	: Opens 4D DAT file for parsing and GCI file or a gcj (.gcj) for read using filename without extension.
//          : JPEG gci's (gcj) can be used by using  filename with extension 'gcj'
//
...
//      	: gfx.Open4dGFX(project_JPEG);
// Notes 	: Loads a 4D DAT array and GCI DATA array for reading 4D widgets
//       	: Loads a 4D JPEG array for reading 4D widgets when just single array parameter uesed
//

void gfx.Open4dGFX(JPEGa, FLsize), void; // Loads a 4D jpeg array for reading 4D widgets
// Syntax	: gfx.Open4dGFX(DATa, DATlen, GCIa, GCIlen);
// Usage	: gfx.Open4dGFX(project_dat, project_dat_size, project_gci, project_gci_size);
//      	: gfx.Open4dGFX(project_JPEG, project_JPEG_size);
// Notes 	: Loads a 4D DAT array and GCI DATA array for reading 4D widgets
//       	: Loads a 4D JPEG array for reading 4D widgets when just single array parameter uesed
//

void gfx.Open4dGFXtoPSRAM(file4d), void; // Loads 4D DAT and GCI files into PSRAMM
// Syntax	: gfx.Open4dGFXtoPSRAM(file4d);
// Usage	: gfx.Open4dGFXtoPSRAM("project");
//      	: gfx.Open4dGFXtoPSRAM("project.gcj");
// Notes	: Loads 4D DAT and GCI files into PSRAM. Using filename with extension loads JPEG gci.
//
...
void gfx.DrawFrameBufferAreaScaleRotate(fb, fx, fy, fw, fh, deg, scW, scH, mirX, mirY, [, altX, altY]), void; Draw image with scaling, rotate and mirror from frame buffer fb
// Syntax	: gfx.DrawFrameBufferAreaScaleRotate(fb, fx, fy, fw, fh, deg, scW, scH, mirX, mirY);
//			: gfx.DrawFrameBufferAreaScaleRotate(fb, fx, fy, fw, fh, deg, scW, scH, mirX, mirY, altx, alty);
// Usage	: gfx.DrawFrameBufferAreaScaleRotate(2, 0, 0, 100, 50, 180, 100, 50, true, true);
// 			: gfx.DrawFrameBufferAreaScaleRotate(2, 0, 0, 90, 90, 90, 90, 90, true, true, 90, 0);
// Notes	: Draw a rotated, scaled image from frame buffer fb using scW - scaled width and scH scaled Height and angle rotation.
//			: Mirror can be enabled with setting mirX and mirY to true for x and y mirror.
//			: Image location at currently specified position with altx, alty
//

void gfx.DrawJPEGinFile(tfile, tpos, tfsize, x, y, forceSoft, altw, alth), void; Draw a JPEG image embedded in an opened file.
// Syntax	: gfx.DrawFrameBufferAreaScaleRotate(DrawJPEGinFile(tfile, tpos, tfsize, x, y, forceSoft, altw, alth));
// Usage	: gfx.DrawFrameBufferAreaScaleRotate(myFile, 7884, 81920, 0, 0, true, 800, 800);
// Notes	: Draw a JPEG image embedded in an opened file.
//			: Can also be an opened file containing just a single JPEG at position 0 and size of file.
//

void gfx.UserImageAsRoundRect(ui, radius){, void; // Draw a single frame GCI widget as Rounded Rectangle
// Syntax	: gfx.UserImageAsRoundRect(ui, radius);
// Usage	: gfx.UserImageAsRoundRect(iImage1, 10);
// Notes 	: Draw a single frame GCI widget in the default location or in new location using ImageSetWord command
//

void gfx.UserImagesAsRoundRect(ui, frame, radius){, void; // Draw a UserImages GCI widget as Rounded Rectangle
// Syntax	: gfx.UserImagesAsRoundRect(ui, frame, radius);
// Usage	: gfx.UserImagesAsRoundRect(iImage1, frame, 10);
// Notes 	: Draw a UserImage GCI widget in the default location or in new location using ImageSetWord command
//
...
void gfx.SelectDataSourceGCI(gcImage), void; // Select the source for image data to be specified image in a GCI file
...
//			: frame   - Optional, frame numbe of image set
//
...
void gfx.AngularGauge(val, x, y, sa, ea, min, max, gaugeRadius, gaugeThickness, gaugeLOcolorLO, gaugeLOcolorHI, gaugeMIDcolorLO, gaugeMIDcolorHI, gaugeHIcolorLO, gaugeHIcolorHI, midVal, hiVal, roundEnds), void; // Draw an angular gauge
// Syntax	: gfx.AngularGauge(val, x, y, sa, ea, min, max, gaugeRadius, gaugeThickness, gaugeLOcolor, gaugeHIcolor, roundEnds);
// Usage	: gfx.AngularGauge(50, 250, 250, 180, 270, 0, 100, 90, 10, BLUE, RED, false);
// Notes	: Draw an angular gauge
//			:   val             - Value to be shown on gauge
...
//			:   gaugeRadius     - internal radius of arc
//			:   gaugeThickness  - Thickness of angular gauge
//			:   gaugeLOcolorLO  - Low value color of gauge low range
//			:   gaugeLOcolorHI  - High value color of gauge low range
//			:   gaugeMIDcolorLO - Low value color of gauge medium range
//			:   gaugeMIDcolorHI - High value color of gauge medium range
//			:   gaugeHIcolorLO  - Low value color of gauge high range
//			:   gaugeHIcolorHI  - High value color of gauge high range
//			:	midVal			- Threshold of medium range
//			:	hiVal			- Threshold of high range
//			:   roundEnds       - True if arc has round ends
//
...
//			:   gaugeRadius     - internal radius of arc
//			:   gaugeThickness  - Thickness of angular 
//			:	tickW			- Thickness of ticks
//			:   gaugeLOcolor    - Low value color of gauge
//			:   gaugeHIcolor    - High value color of gauge
...
void gfx.imageShow(uisnb), void; // Show GCI image as given be index.
// Syntax	: gfx.imageShow(uisnb);
...
int16_t gfx.touch_DeltaX(), 1; // Returns the latest touch X distance in pixels after the last gfx.touch_Update
// Syntax	: gfx.touch_touch_DeltaX();
// Usage	: gfx.touch_touch_DeltaX();
// Notes 	: Returns the latest touch X distance in pixels after the last gfx.touch_Update
//			: This function is only available for capacitive touch displays
//

int16_t gfx.touch_DeltaY(), 1; // Returns the latest touch Y distance in pixels after the last gfx.touch_Update
// Syntax	: gfx.touch_touch_DeltaY();
// Usage	: gfx.touch_touch_DeltaY();
// Notes 	: Returns the latest touch Y distance in pixels after the last gfx.touch_Update
//			: This function is only available for capacitive touch displays
//

int16_t gfx.touch_GetCanvasTouched(), 1; // Returns which Canvas window is touched
// Syntax	: gfx.touch_GetCanvasTouched();
// Usage	: gfx.touch_GetCanvasTouched();
// Notes 	: Returns CANVAS_BUFFER_TOUCHED if canvas buffer is touched or CANVAS_BUFFER_ARGB_TOUCHED
//		: if canvas buffer argb is touched.
//
...
void gfx.PPAflushArea(srcBuff, srcW, srcH, x, y, bswap, fmt24), void; // Flushes LVGL buffer to display area
// Syntax	: gfx.FlushArea(srcBuff, srcW, srcH, x, y, bswap, fmt24);
// Usage	: gfx.FlushArea(lvglBuf, lvglWidth, lvglHeight, lvglX, lvglY, true, false);
// Notes	: Can be used out of LVGL for sending image data to display.
//

void gfx.SetMaxWidgets(mw), void // Sets the maximum widget allocation for GCI widgets
// Syntax	: gfx.SetMaxWidgets(mw);
// Usage	: gfx.SetMaxWidgets(200);
// Notes	: Only necessary if user code expects to change GCI / DAT in code.
//

void gfx.SetMaxJPEGWidgets(mw), void // Sets the maximum JPEG widget allocation for GCJ widgets
// Syntax	: gfx.SetMaxJPEGWidgets(mw);
// Usage	: gfx.SetMaxJPEGWidgets(200);
// Notes	: Only necessary if user code expects to change GCJ in code.
//
...
void gfx.SpeakerVolume(v), void; // Adjust the volume of the audio codec
// Syntax	: gfx.SpeakerVolume(v);
// Usage	: gfx.SpeakerVolume(78);
// Notes	: Adjust the volume of the audio codec, 0 to 100
//			: v - Volume, 0 to 100
//

void gfx.AudioInit(), void; // Initialize audio codec
// Syntax	: gfx.AudioInit();
// Usage	: gfx.AudioInit();
// Notes	: Initialize audio codec.
//