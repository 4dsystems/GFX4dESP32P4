...
void gfx4desp32P4::uSDmount(){
    ...
    if (SD_MMC.begin("/sdcard", true, false, 50000))
#else
		if (SD_MMC.begin("/sdcard", false, false, 32000, 5))
#endif
    {
        sdok |= true;
    }
...
*/
/****************************************************************************/
void gfx4desp32P4::Cls(uint16_t color) {
    FillScreen(color);
    cursor_x = textXmin;
    cursor_y = 0;
    scrolled = false;
...
void gfx4desp32P4::TextCursorXYfromScrollWindow() {
	...
	cursor_y = scroll_window_store_x2;
}
...
/****************************************************************************/
/*!
  @brief  Draw Userimages with scaling and mirror defined by output window 
  @param  ui - UserImages ID
  @param  frame - UserImages frame
...
*/
/****************************************************************************/
void gfx4desp32P4::UserImagesScale(uint16_t ui, int frame, int scW, int scH, bool mirX, bool mirY, int altx, int alty){
	if (altx == 0x7fff && alty == 0x7fff) {
		UserImageSRM(ui, 8, frame, scW, scH, 0, mirX, mirY, tuix[ui], tuiy[ui]);
	} else {
		UserImageSRM(ui, 8, frame, scW, scH, 0, mirX, mirY, altx, alty);
	}
}

/****************************************************************************/
/*!
  @brief  Draw Userimages with Rotation and mirror 
  @param  ui - UserImage ID
  @param  frame - UserImages frame
  @param  angle - Width of scaled image only 0, 90, 180, 270 degrees allowed
  @param  mirX - true if miiror on the X axis
  @param  mirY - true if miiror on the Y axis
  @param  altx - Alternative x position in pixels
  @param  alty - Alternative y position in pixels
*/
/****************************************************************************/
void gfx4desp32P4::UserImagesRotate(uint16_t ui, int frame, int angle, bool mirX, bool mirY, int altx, int alty){
	int rtw = tuiw[ui];
	int rth = tuih[ui];
	if (altx == 0x7fff && alty == 0x7fff) {
		UserImageSRM(ui, 8, frame, rtw, rth, angle, mirX, mirY, tuix[ui], tuiy[ui]);
	} else {
		UserImageSRM(ui, 8, frame, rtw, rth, angle, mirX, mirY, altx, alty);
	}
}

/****************************************************************************/
/*!
  @brief  Draw Userimages with Scale, Rotation and mirror defined by output window 
  @param  ui - UserImage ID
  @param  frame - UserImages frame
  @param  scW - Width of scaled image
  @param  scH - Height of scaled image
  @param  angle - Rotation angle, only 0, 90, 180, 270 degrees allowed
  @param  mirX - true if miiror on the X axis
  @param  mirY - true if miiror on the Y axis
  @param  altx - Alternative x position in pixels
  @param  alty - Alternative y position in pixels
*/
/****************************************************************************/
void gfx4desp32P4::UserImagesScaleRotate(uint16_t ui, int frame, int scW, int scH, int angle, bool mirX, bool mirY, int altx, int alty){
	int rtw = scW;
	int rth = scH;
	if (altx == 0x7fff && alty == 0x7fff) {
		UserImageSRM(ui, 8, frame, rtw, rth, angle, mirX, mirY, tuix[ui], tuiy[ui]);
...
/****************************************************************************/
/*!
  @brief  Draw single Userimage with scaling and mirror defined by output window 
  @param  ui - UserImage ID
  @param  scW - Width of scaled image
  @param  scH - Height of scaled image
  @param  mirX - true if miiror on the X axis
...
*/
/****************************************************************************/
void gfx4desp32P4::UserImageScale(uint16_t ui, int scW, int scH, bool mirX, bool mirY, int altx, int alty){
	if (altx == 0x7fff && alty == 0x7fff) {
		UserImageSRM(ui, 6, 0, scW, scH, 0, mirX, mirY, tuix[ui], tuiy[ui]);
	} else {
		UserImageSRM(ui, 6, 0, scW, scH, 0, mirX, mirY, altx, alty);
	}
}

/****************************************************************************/
/*!
  @brief  Draw single Userimage with Rotation and mirrorw 
  @param  ui - UserImage ID
  @param  angle - Width of scaled image only 0, 90, 180, 270 degrees allowed
  @param  mirX - true if miiror on the X axis
  @param  mirY - true if miiror on the Y axis
...
*/
/****************************************************************************/
void gfx4desp32P4::UserImageRotate(uint16_t ui, int angle, bool mirX, bool mirY, int altx, int alty){
	int rtw = tuiw[ui];
	int rth = tuih[ui];
	if (altx == 0x7fff && alty == 0x7fff) {
		UserImageSRM(ui, 6, 0, rtw, rth, angle, mirX, mirY, tuix[ui], tuiy[ui]);
	} else {
		UserImageSRM(ui, 6, 0, rtw, rth, angle, mirX, mirY, altx, alty);
	}
}

/****************************************************************************/
/*!
  @brief  Draw single Userimage with Scale, Rotation and mirror defined by output window 
  @param  ui - UserImage ID
  @param  scW - Width of scaled image
  @param  scH - Height of scaled image
  @param  angle - Rotation angle, only 0, 90, 180, 270 degrees allowed
...
*/
/****************************************************************************/
void gfx4desp32P4::UserImageScaleRotate(uint16_t ui, int scW, int scH, int angle, bool mirX, bool mirY, int altx, int alty){
	int rtw = scW;
	int rth = scH;
	if (altx == 0x7fff && alty == 0x7fff) {
		UserImageSRM(ui, 6, 0, rtw, rth, angle, mirX, mirY, tuix[ui], tuiy[ui]);
...
/****************************************************************************/
/*!
  @brief  Support function for Userimage / Userimages SRM functions
  @param  ui - UserImage ID
  @param  ofst - 6 or 8 offset of image data in the GCI object
  @param  frame - frame number if a Userimages object
  @param  scW - Width of scaled image
  @param  scH - Height of scaled image
  @param  angle - Rotation angle, only 0, 90, 180, 270 degrees allowed
  @param  mirX - true if miiror on the X axis
...
*/
/****************************************************************************/
void gfx4desp32P4::UserImageSRM(uint16_t ui, int ofst, int frame, int scW, int scH, int angle, bool mirX, bool mirY, int altx, int alty){
	...
	//	rtw = scH;
	//}	
	uint32_t pos = tuiIndex[ui] + ofst;
	if (ofst == 8) {
		pos += (((tuiw[ui] * tuih[ui]) << 1) * frame);
	}
    if (GCItype == GCI_SYSTEM_USD && (userImag)){
		GCIreadToBuff(pos, (tuiw[ui] * tuih[ui]) << 1);
		drawPPAobject(psRAMbuffer1, tuiw[ui], tuih[ui], rtw, rth, angle, mirX, mirY, altx, alty, true);
	}		
	if (GCItype == GCI_SYSTEM_PROGMEM && (gcidatArray)){
		drawPPAobject(GCIarray + pos, tuiw[ui], tuih[ui], rtw, rth, angle, mirX, mirY, altx, alty, true);
	}
    ScrollEnable(setemp);
}

void gfx4desp32P4::drawPPAobject(const uint8_t *sourceBuf, float sWidth, float sHeight, float scWidth, float scHeight, int deg, bool mirX, bool mirY, int x, int y, bool bSwap){
  if (ppaInit == false){
	  scaledBuffer = (uint16_t*)heap_caps_aligned_alloc(64, getWidth() * getHeight() * 2, MALLOC_CAP_DMA | MALLOC_CAP_SPIRAM);
	  ppaInit = true;
  }
  uint16_t* sbuf = (uint16_t*)sourceBuf;
  if(ppaAccelerator.scaleRotateImage(sbuf, sWidth, sHeight, scaledBuffer, scWidth, scHeight, bSwap, deg, mirX, mirY)){
	  if (deg == 90 || deg == 270){
		  SetGRAM(x, y, x + scHeight - 1, y + scWidth - 1);
	  } else {
	      SetGRAM(x, y, x + scWidth - 1, y + scHeight - 1);
      }
	  WrGRAMs(scaledBuffer, scWidth * scHeight);
  }
}

void gfx4desp32P4::drawPPAobject(uint8_t *sourceBuf, float sWidth, float sHeight, float scWidth, float scHeight, int deg, bool mirX, bool mirY, int x, int y, bool bSwap){
  if (ppaInit == false){
	  scaledBuffer = (uint16_t*)heap_caps_aligned_alloc(64, getWidth() * getHeight() * 2, MALLOC_CAP_DMA | MALLOC_CAP_SPIRAM);
	  ppaInit = true;
  }
...
*/
/****************************************************************************/
void gfx4desp32P4::DrawFrameBufferAreaScaleRotate(int fb, int fx, int fy, float fw, float fh, int deg, float scW, float scH, bool mirX, bool mirY, int altX, int altY){
    if (ppaInit == false){
	  scaledBuffer = (uint16_t*)heap_caps_aligned_alloc(64, getWidth() * getHeight() * 2, MALLOC_CAP_DMA | MALLOC_CAP_SPIRAM);
	  ppaInit = true;
    }	
	...
	ppaAccelerator.scaleRotateImageFB(pto, 800, 1280, fx, fy, fw, fh, scaledBuffer, scW, scH, false, deg, mirX, mirY);
    //if (ideg == 90 || ideg == 270){
	//	SetGRAM(x, y, x + scH - 1, y + scW - 1);
	//} else {
	    SetGRAM(x, y, x + scW - 1, y + scH - 1);
    //}
	WrGRAMs(scaledBuffer, scW * scH);
} 	
...
void gfx4desp32P4::PrintImage(uint16_t ui) {
    ...
    boolean off = false;
    int cuiw = iwidth;
    if ((cursor_x + iwidth - 1) >= textXmax) {
        cuiw = iwidth - ((cursor_x + iwidth - 1) - textXmax) - 1;
        off = true;
    }
...
void gfx4desp32P4::TWprint(String istr) { TWstring2write(istr); }

void gfx4desp32P4::TWprint(char* istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWprint(int8_t istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWprint(uint8_t istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWprint(int16_t istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWprint(uint16_t istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWprint(uint istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWprint(int32_t istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWprint(uint32_t istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWprint(int64_t istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWprint(uint64_t istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWprint(float istr) { TWstring2write(String(istr)); }

void gfx4desp32P4::TWstring2write(String istr) {
    char tistr;
...
void gfx4desp32P4::RectangleFilledX(int x0, int y0, int x1, int y1, int32_t color) {
    ...
    if (fB) {
		...
		ClipWindow(tclipx1, tclipy1, tclipx2, tclipy2);
		Clipping(ON);
		if (!tclippingON) Clipping(OFF);
    }
    else {
...
void gfx4desp32P4::imageSetWord(uint16_t img, byte controlIndex, int val1,
    int val2) {
    switch (controlIndex) {
    case IMAGE_XPOS:
        tuix[img] = val1;
...
void gfx4desp32P4::PrintImageFile(String ifile) {
    ...
    boolean off = false;
    int cuiw = Piwidth;
    if ((cursor_x + Piwidth - 1) >= textXmax) {
        cuiw = Piwidth - ((cursor_x + Piwidth - 1) - textXmax) - 1;
        off = true;
    }
...
void gfx4desp32P4::GCIreadToBuff(uint32_t Index, uint32_t len) {
    switch (GCItype) {
    case GCI_SYSTEM_USD:
        userImag.seek(Index);
...
*/
/****************************************************************************/
void gfx4desp32P4::ShowAnim(int animIndex){
  int arraypos = 7 * animIndex;
  DrawToframebuffer(0);
